fn main(){
  ...
  */ 
  ...
  // 设置了+-正负符号后，数字的对齐方式就只能是右对齐，如果设置对齐方式，正负负号会被当成填充字符。
  println!("hello,{:+<6}!", 9527); // "hello,9527++!"，+符号被当成填充字符了
  // 0的使用
  println!("hello,{:06}!", -9527); // "hello,-09527!"，设置了0的标识后，会在数字前面填充上前导0，如果设置了+符号标识或者数字为负数，+-符号也会占用一个长度。
  // #的使用
  // 具体看下下面的表
  /*
  格式    实现trait           说明                                           加#号
  -----------------------------------------------------------------------------------------------------
  {}     fmt::Display       大部分原生数据类型都实现，但不期待所有类型实现         禁止
  {:?}   fmt::Debug         在没有实现Display的情况下，可以通过实现它来打印       {:#?}，rust以较友好的方式打印
  {:b}   fmt::Binary        2进制                                           {:#b}在打印信息前加0b
  {:o}   fmt::Octal         8进制                                           {:#o}在打印信息前加0o
  {:x}   fmt::LowerHex      16进制小写                                       {:#x}在打印信息前加0x
  {:X}   fmt::UpperHex      16进制大写                                       {:#X}在打印信息前加0X
  {:x?}  fmt::Debug         无                                              禁止  
  {:X?}  fmt::Debug         无                                              禁止
  {:p}   fmt::Pointer       指针地址                                         禁止
  {:e}   fmt::LowerExp      科学记数法，小写e                                 禁止
  {:E}   fmt::UpperExp      科学记数法，大写E                                 禁止
  */ 
  /**
   * 五：.小数点使用
  */
  // 首先，.的使用是针对浮点数的，特殊的，浮点数的字符串会有特殊的逻辑。其它数据类型下，.的使用是无效的。
  println!("hello,{:.3}!", 3.2335); // "hello,3.233!"
  println!("hello,{:.3}!", 4.2335); // "hello,4.234!"
  // 可以看出来，上面的情况下，小数部分超过设置长度的会被截取，截取的规则不是简单的四舍五入
  // https://users.rust-lang.org/t/formatting-floats-and-rounding/10235/4 
  // 从这个链接能看到这个问题的讨论，这与小数的表示方式有关，感兴趣的可以详细去了解一下
  // 针对.，后面的长度值也可以通过[索引|命名]加$符号的方式来表示
  println!("hello,{:.1$}!", 3.2335, 3);
  println!("hello,{:.len$}!", 3.2335, len = 3);
  // .后面还可以加*符号，其会先于本身占位占用一个参数位，如果对应本身的数据占位没有使用[索引|命名]，会将下一个参数位当做数据占位
  println!("hello,{:.*}!", 3, 3.2335); // 这里*对应了第一个参数位，3.2335作为数据位
  println!("hello,{0:.*}!", 3.2335, 3); // 只有两个参数时比较特殊，通过改变数据占位符，.*变成了剩下的那个参数的值
  println!("{},{1:.*}!", "hello", 3.2335, 3); // 这种情况下会报错，.*和数据位1都对应着第二个参数，而不是.*对应着剩余的第三个参数
  /**
   * 最后看一下，整个格式化规则
   */
  println!("format := '{' [ argument ] [ ':' format_spec ] '}'");
  println!("argument := integer | identifier");
  println!("format_spec := [[fill]align][sign]['#']['0'][width]['.' precision][type]");
  /**
   * 还有如何转义
   */
  println!("{{"); // 输出"{"
